...
# Pattern to find and replace the result view section
old_pattern = r'''                \) : viewMode === 'result' && gradedPreviewUrl \? \(
                  <div className="w-full h-full border-4 border-black shadow-neo overflow-hidden bg-black">
                    <img\s+
                      src=\{gradedPreviewUrl\}\s+
                      alt="AI Refined Result"\s+
                      className="w-full h-full object-contain"\s+
                    />
                  </div>'''

new_section = '''                ) : viewMode === 'result' && (srcPreviewUrl || gradedPreviewUrl) ? (
                  <div className="w-full h-full border-4 border-black shadow-neo overflow-y-auto bg-gray-50 p-4">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      ...
                      {gradedPreviewUrl && (
                        <div className="border-2 border-black shadow-brutal bg-white">
                          <div className={`border-b-2 border-black px-3 py-2 font-mono text-sm font-bold flex items-center gap-2 ${
                            isRefined ? 'bg-purple-200' : 'bg-yellow-200'
                          }`}>
